...
export class World<T extends Agent2D = Agent2D> {
   ...
   * @description 应用相互作用力
   */
  private async applyInteraction(agent: T) {
    ...
    this.forEachAgent(targetAgent => {
      ...
      const targetKey = `${targetAgent.uuid}:${agent.uuid}`
      if (reactedSet.has(key) || reactedSet.has(targetKey)) return
