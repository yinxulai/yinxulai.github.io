...
/**
 * 神经网络中的连接每个连接具有权重、源节点和目标节点还具有内部状态（相对于特定输入的误差导数），
 * 在反向传播运行后会更新该状态
 */
export class Link {
  id: string // 连接的唯一标识符
   ...
   * @param source 源节点
   * @param dest 目标节点
   * @param regularization 计算此权重的正则化函数如果为null，则不应用正则化
 ...
 * 构建神经网络
 *
 * @param networkShape 网络的形状例如，[1, 2, 3, 1] 表示网络将具有一个输入节点，第一个隐藏层有2个节点，第二个隐藏层有3个节点，输出层有1个节点
 * @param activation 每个隐藏节点的激活函数
 * @param outputActivation 输出节点的激活函数
 * @param regularization 计算网络中给定权重（参数）的正则化函数如果为 null，则不进行正则化
 * @param inputIds 输入节点的 id 列表
 * @param initZero 是否将节点的初始值设置为零默认为 false
 */
...
/**
 * 通过提供的输入数组对神经网络进行正向传播该方法修改网络的内部状态，
 * 即网络中每个节点的总输入和输出
 *
 * @param network 神经网络
 * @param inputs 输入数组其长度应与输入层中的节点数匹配
 * @return 网络的最终输出
 */
