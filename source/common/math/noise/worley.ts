...
type DistanceFunc = (p1: P, p2: P) => number
...
export class Worley {
    ...
    static EuclideanDistance(p1: P, p2: P) {
        return Worley.d(p1, p2).reduce((sum, x) => sum + (x * x), 0)
    }

    static ManhattanDistance(p1: P, p2: P) {
    ...
    static insert(arr: number[], value: number) {
        let temp
    ...
    noise(input: P, distanceFunc: DistanceFunc) {
        let lastRandom: number,
            numberFeaturePoints: number,
            randomDiff: P = { x: 0, y: 0, z: 0 },
            featurePoint: P = { x: 0, y: 0, z: 0 }
