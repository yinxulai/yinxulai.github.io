...
export function useDraw(canvas: Ref<HTMLCanvasElement | undefined>) {
  ...
  watch(canvas, () => {
    ...
    canvas.value.addEventListener('mouseout', close, { passive: true })
    canvas.value.addEventListener('mousedown', start, { passive: true })
    canvas.value.addEventListener('mouseleave', close, { passive: true })
